...
# === Cargar los datos ===
df = pd.read_csv('../datasets/processed/dataset-vehiculos-limpio.csv')  # Reemplaza con tu ruta si es diferente
...
# === Comparación precios reales vs. predichos por marca ===
X_test_copy = X_test.copy()
X_test_copy['precio_real'] = y_test
X_test_copy['precio_predicho'] = y_pred

# Agrupar por marca y comparar medias
grouped = X_test_copy.groupby('make')[['precio_real', 'precio_predicho']].mean().sort_values(by='precio_real', ascending=False)

# Gráfico de comparación completo
plt.figure(figsize=(14, 8))
ax = grouped.plot(kind='bar', figsize=(14, 8))
plt.ylabel("Precio medio (€)")
plt.title("Comparación del precio real vs. predicho por marca")
plt.xticks(rotation=90)
plt.tight_layout()
plt.legend(title='Precio', labels=['Real', 'Predicho'])
plt.savefig('../graficos/modelo-predictivo/precio_real_vs_predicho_por_marca.png', dpi=300)
plt.show()
